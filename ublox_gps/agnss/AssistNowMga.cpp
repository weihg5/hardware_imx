 ...
 *
 ******************************************************************************
 * $Id: AssistNowMga.cpp 96420 2015-05-28 11:25:23Z marcel.baracchi $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/release_v3.10/gps/agnss/AssistNowMga.cpp $
 *****************************************************************************/
...
bool CAssistNowMga::extractMgaIniTimeUtcMsg(unsigned char * buf, size_t size, bool clearMsg)
{
	...
	bool result=true;
	BUFC_t *tmpMsg=NULL;
	if(verifyUbxMsgsBlock(buf, size, NULL, 0, &tmpMsg)>0)
	{
		if( tmpMsg[0].p             // Valid pointer
		 && tmpMsg[0].i    == 24+8  // Valid UBX-MGA-INI-TIME_UTC length
		 && tmpMsg[0].p[2] == 0x13  // MGA
		 && tmpMsg[0].p[3] == 0x40  // INI
	...
	switch(evtType)
	{
		...
		case MGA_PROGRESS_EVT_TERMINATED:
		{
			...
			if(*stopReason!=TERMINATE_HOST_CANCEL)
			{
				switch(*stopReason)
				{
					case TERMINATE_RECEIVER_NACK:
					{
						snprintf(msgTxt + strlen(msgTxt), TMP_STR_LEN - strlen(msgTxt),
								 "NACK from receiver. Data received, but receiver had a problem processing it. Consider all data invalidated.");
						break;
					}
