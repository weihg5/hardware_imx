 ...
 *
 ******************************************************************************
 * $Id: ComThread.cpp 96362 2015-05-27 18:18:51Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/release_v3.10/gps/agnss/com/ComThread.cpp $
 *****************************************************************************/
		...
		for(size_t i=0; i < arrFd.num; ++i)
		{
			      ...
			      && notify(pos, DATA_AVAILABLE) )
			{
				...
				if(bytes_read > 0)
				{
					...
					process(pos, pTmp, (size_t) bytes_read);
					free(pTmp);
				}
