 ...
 *
 ******************************************************************************
 * $Id: helperFunctions.cpp 96357 2015-05-27 17:25:28Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/release_v3.10/gps/agnss/helper/helperFunctions.cpp $
 *****************************************************************************/
...
bool isAllowedUbxMsg(unsigned char const * buf, int size, UBX_MSG_TYPE const * allowed, size_t allowed_num, bool allowWildCards /* = false */)
{
	...
	for(size_t i=0; i < allowed_num && !allowedMsg ; ++i)
	{
		...
		else if( allowWildCards ) // Are wild cards allowed?
		{
...
/*! This function will verify if the passed data contains a valid 
    ...
    \param pData      : A pointer to the buffer from which the data should
                        copied to the header structure if it is valid ALP
                        data. Must not be NULL.
    \param iData      : The size of the data in bytes pointed to by pData
                        Must not be 0.
...
ssize_t availRead( int fd, unsigned char ** buf, size_t maxSize /* = 0 */ )
{
	...
	if(ioctl(fd, FIONREAD, &bytes_available) == 0)
	{
		if(bytes_available > 0)
		{
			...
			// the buffer
			if(size==0 || size > (size_t) bytes_available)
               ...
               , int timeoutMs)
{
	...
	// or is it just a signaliisation to
...
/*! Calculate the 8-bit Fletcher checksum for the specified buffer
    ...
    \param            : The number of bytes contained in data. Must
                        not be 0.
	\param            : Checksum of previous partial calculation.
                        Optional
...
int8_t decodeAgnssf00(unsigned char const *raw, size_t rawSize, BUF_t * bufs, int8_t numBufs)
{
	...
	if(!raw || !bufs || numBufs<=0 || rawSize > SSIZE_MAX || rawSize < minRawSize)
		return HELPER_ERR_INVALID_ARGUMENT;
                 ...
                 , const void* pData1, size_t iData1)
{
	...
	size_t outCount=0;
	unsigned char * outBuf = (unsigned char*) malloc(iData+CRC_SIZE+HEADER_SIZE);
    ...
    outBuf[2] = classID;
    outBuf[3] = msgID;
    outBuf[4] = (unsigned char)iData;
    outBuf[5] = (unsigned char)(iData >> 8);
    checksum=calculateChecksum(&outBuf[2], HEADER_SIZE-2, &checksum);
	outCount=HEADER_SIZE;

	// Is there pData0? If yes, use it as payload p1
	if(pData0)
	{
		memcpy(outBuf+outCount, pData0, iData0);
		checksum=calculateChecksum((const unsigned char*)pData0, iData0, &checksum);
		outCount+=iData0;
	}

	// Is there pData1? If yes, use it as payload p1/p2
	if(pData1)
	{
		memcpy(outBuf+outCount, pData1, iData1);
		checksum=calculateChecksum((const unsigned char*)pData1, iData1, &checksum);
		outCount+=iData1;
	}

	// Create CRC
	outBuf[outCount]=checksum >> 8;
	++outCount;
	outBuf[outCount]=checksum;
	++outCount;

	*buf=outBuf;
	assert(isUbxMessage(*buf, outCount, false));
	return outCount;
}

/*! Make sure the nanosecond part is >=0 and <= 999999999
    \param time     : The time that has to be normalized
*/
void normalizeTime(struct timespec * unTime)
{
	if(unTime)
	{
		// Add whole seconds to the seconds part and substract
...
time_t getEarliestSaneTime(struct timespec * earliestTime /* = NULL */)
{
	if(earliestTime)
	{
		earliestTime->tv_sec=EARLIEST_TIME_S;
		earliestTime->tv_nsec=0;
	}
	return EARLIEST_TIME_S;
}

void includeLeapSeconds(ACCTIME_t * accTime)
{
	if(accTime && !accTime->leapSeconds)
	{
		accTime->time.tv_sec -= LEAP_SECONDS_UNIX;
                     ...
                     , POS_t * pos /* = NULL */ )
{
	...
	if(accTime && pay->flags & GPS_UBX_AID_INI_U5__FLAGS_TIME_MASK)
	{
		...
		leapSecMs=MSEC_PER_SEC * LEAP_SECONDS_GPS;
		uint64_t timeGpsMs=((uint64_t)pay->wn)*oneWeekMs+(uint64_t)pay->tow-leapSecMs;
		// convert to Unix time
		timeGpsMs	   += (int64_t) GPS_EPOCH_UNIX_S*MSEC_PER_SEC; 
                    ...
                    , POS_t const * pos /* = NULL */ )
{
	...
	if(accTime)
	{
		...
		// encode the time
		if(timeGpsMs >= (uint64_t) GPS_EPOCH_UNIX_S*MSEC_PER_SEC)
		{
			// convert Unix time to GPS time
			//lint -e(571)
			timeGpsMs	   -= (uint64_t) GPS_EPOCH_UNIX_S*MSEC_PER_SEC;
