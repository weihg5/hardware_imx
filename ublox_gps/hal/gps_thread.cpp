 ...
 *
 ******************************************************************************
 * $Id: gps_thread.cpp 96291 2015-05-26 19:41:34Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/release_v3.10/gps/hal/gps_thread.cpp $
 *****************************************************************************/
...
void ubx_thread(void *pThreadData)
{
    ...
    SSL_library_init();
	UBX_LOG(LCAT_VERBOSE, "SSL initialised");
#endif //ifdef SUPL_ENABLED
                ...
                if (iSize)
                {
                    ...
                    while (parser.Parse(pProtocol, pMsg, iMsg))
                    {
						...
						if (pProtocol == &protocolUBX)
						{
							...
							{
								UBX_LOG(LCAT_VERBOSE, "MSG UBX RX: %s (size %d)", pStr, iMsg);
								free(pStr);
							}
						...
						{
                            UBX_LOG(LCAT_VERBOSE, "MSG UNKNOWN size %d", iMsg);
							...
							{
								UBX_LOG(LCAT_VERBOSE, "MSG UNKNOWN RX: %s (size %d)", pStr, iMsg);
								free(pStr);
							}
