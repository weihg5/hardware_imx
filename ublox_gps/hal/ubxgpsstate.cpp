 ...
 *
 ******************************************************************************
 * $Id: ubxgpsstate.cpp 96104 2015-05-21 11:39:07Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/release_v3.10/gps/hal/ubxgpsstate.cpp $
 *****************************************************************************/
...
#define SERPORT_DEFAULT               "/dev/ttyO3" //!< Default receiver interface
		...
		switch( action )
		{
			...
			case CAgnss::DOWNLOAD:
			{
				...
				if(!isCurrentChecksum(service, dataId))
				{
					...
					if(service!=CAgnss::RECV_AID_STATE)
					{
						UBX_LOG( LCAT_VERBOSE, "New %s data injected. Scheduling transfer."
		                       , (agnssActionToString(action))
							   , (agnssServiceTypeToString(service)));
				...
				{
					UBX_LOG( LCAT_VERBOSE, "%sed %s data identical to already transferred one. Do nothing."
		                   , (agnssActionToString(action))
						   , (agnssServiceTypeToString(service)));
				}
				break;
