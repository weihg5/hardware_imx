 ...
 *
 ******************************************************************************
 * $Id: rrlpmanager.cpp 82140 2014-06-17 12:20:24Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/release_v3.10/gps/supl/rrlpmanager.cpp $
 *****************************************************************************/
	...
	if (svCount > 0)
	{
		...
		for(int i = 0; i < svCount; i++)
		{
			...
			if ((adjustedDoppler >= -32768) && (adjustedDoppler <= 32767))
			{
				...
				assert(ret == 0);
//lint -e{429} removed Custodial pointer 'pMsrElement' (line 386) has not been freed or returned
			}
			else
			{
...
//lint -e{593} removed Custodial pointer 'pMsrSetElement' (line 342) possibly not freed or returned
	}
...
static void fillRefTime(GPS_UBX_AID_INI_U5__t *pHandl, int tow, int wn)
{
    ...
    pHandl->tow = (unsigned int) tow * 80 + TIME_REFERENCE_OFFSET_MILLS;

    /* Write the wn value (1024 offset!) */
    pHandl->wn = (U2) wn + 1024;
