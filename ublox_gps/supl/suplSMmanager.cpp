 ...
 *
 ******************************************************************************
 * $Id: suplSMmanager.cpp 82140 2014-06-17 12:20:24Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/hmw/android/release/release_v3.10/gps/supl/suplSMmanager.cpp $
 *****************************************************************************/

/*!
  \file
  \brief  SET state Machine administrator
...
//			UBX_LOG(LCAT_VERBOSE, "Open SUPL handle - Listerning for data on %d", fd);
            FD_SET(fd, pRfds);
...
static void freeSuplHandler(suplHandler_t *pHandler)
{
	...
	if (pHandler->bio != 0)
	{
        ...
        if (pAgps->isTlsActive())
        {
            ...
            {
                UBX_LOG(LCAT_WARNING, "ssl instance is NULL");
            }
        }
		BIO_free_all(pHandler->bio);
		SSL_CTX_free(pHandler->ctx);
		UBX_LOG(LCAT_VERBOSE, "closed socket");
	}
...
static int createUplSession(suplHandler_t *pHandler)
{
    ...
    int original_retries = pAgps->getConnectRetries(); // 5;
    int retry = original_retries;
    bool bHaveConnection = false;

    if (pAgps->isTlsActive())
    {
        UBX_LOG(LCAT_VERBOSE, "**SECURE** TLS connection");
        /* Set up the SSL context */
        if ( (pHandler->ctx = SSL_CTX_new(SSLv23_client_method())) == NULL)
        {
            UBX_LOG(LCAT_ERROR, "Error in creation of new context: %s", ERR_error_string(ERR_get_error(),NULL));
			return -1;
        }

        /* Load the trust store if specified, otherwise skip this step and accept any CA */
		const char* certFileName = pAgps->getCertificateFileName();
		assert(certFileName);
		
        if(*certFileName)
        {
            UBX_LOG(LCAT_VERBOSE, "Loading the certificate %s", certFileName);

            if(!SSL_CTX_load_verify_locations(pHandler->ctx, certFileName, NULL))
            {
                UBX_LOG(LCAT_ERROR, "Error loading trust store %s", certFileName);
                return -1;
            }
        ...
        do
        {
            ...
            {
                UBX_LOG(LCAT_ERROR, "Cannot connect (%d retries)", original_retries - retry);
            }
            else
            {
                ...
                {
                    UBX_LOG(LCAT_VERBOSE, "TLS Session connection failed (%d retries)", original_retries - retry);
                }
                else
                {
            ...
            // Wait a bit, but only if more retries to be done
            if (retry)
                usleep(100000);
        ...
        if (!bHaveConnection)
        {
            UBX_LOG(LCAT_VERBOSE, "Connection failure, exting");
            return -1;
        }
		...
		if (*certFileName)
		{
			...
			if(res != X509_V_OK)
			{
				UBX_LOG(LCAT_ERROR, "Certificate verification error: %ld", res);
				return -1;
			}
		}

        UBX_LOG(LCAT_VERBOSE, "TLS Session established...");
        /* update the handler */
        pHandler->bio = bio;
    }
    else
    {
        UBX_LOG(LCAT_VERBOSE, "Non TLS connection to %s", tmpString);
        ...
        do
        {
            ...
            {
                UBX_LOG(LCAT_VERBOSE, "Non TLS Session connection failed (%d retries)", original_retries - retry);
                pHandler->bio = NULL;
            }
            else
            {
        ...
        UBX_LOG(LCAT_VERBOSE, "Non TLS Session established...");
    }
...
static int connectTimeout(BIO *bio, int sid, int seconds)
{
	...
	do
    {
		...
		UBX_LOG(LCAT_VERBOSE, "# server connecting...");
		...
		{
			/* Hard error? */
            UBX_LOG(LCAT_ERROR, "Cannot connect");
            BIO_free_all(bio);
        ...
        if (fd == -1)
		{
            UBX_LOG(LCAT_ERROR, "BIO isn't initialized in oh_ssl_connect()");
        }
        assert(fd != -1);
...
//lint -e{866} suppress "Unusual use of '' in argument to sizeof"
        ...
        if (err <= 0)
		{
			if (err < 0)
			{
				UBX_LOG(LCAT_ERROR, "error during select()");
			}
			else
			{
				UBX_LOG(LCAT_VERBOSE, "connection timeout");
			}
            BIO_free_all(bio);
