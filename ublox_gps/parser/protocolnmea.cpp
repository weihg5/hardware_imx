 ...
 *
 ******************************************************************************
 * $Id: protocolnmea.cpp 86496 2014-10-24 17:37:48Z fabio.robbiani $
 * $HeadURL: http://svn.u-blox.ch/GPS/SOFTWARE/PRODUCTS/libParser/tags/libParser_v1.03/protocolnmea.cpp $
 *****************************************************************************/
...
{
	if ((pBuffer[2] == 'N') || (pBuffer[2] == 'P') || (pBuffer[2] == 'L'))
	{
		...
		else if (strncmp((char*) &pBuffer[3], "GSV", 3) == 0)	pDatabase->Process(CDatabase::MSG_NMEA_GSV, pBuffer, iSize, ProcessGSV);
...
void CProtocolNMEA::ProcessGSA(unsigned char* pBuffer, int iSize, CDatabase* pDatabase)
{
	...
	// SVs - Pick up from where we left off
	int svsUsed = pDatabase->GetNext<int>(CDatabase::DATA_SATELLITES_USED_COUNT);
	...
	// Accumulated satellites used count
...
{
	// In a multi GNSS environment, we can expect multiple GSV message sets per epoch
	int iMessage;

	int iTotalNumSentences;
	if (GetItem(1, pBuffer, iSize, iTotalNumSentences) && GetItem(2, pBuffer, iSize, iMessage) &&
		(iMessage > 0) && (iTotalNumSentences > 0) && (iMessage <= iTotalNumSentences))
	{
		int itemCount = GetItemCount(pBuffer, iSize);

		int svCount;
		if (GetItem(3, pBuffer, iSize, svCount) && (svCount > 0))
		{
			// Pick up from where we last left off
			int ixInView = pDatabase->GetNext<int>(CDatabase::DATA_SATELLITES_IN_VIEW);

			// Scan through up to 4 sv info field sets
			for (int ix = 0; (ix < 4) && (ixInView < CDatabase::MAX_SATELLITES_IN_VIEW); ix++)
			{
				// svid
				int svid;
				int svInfoPos = 4 + (4 * ix);
				if (svInfoPos == itemCount)
				{
					// Reached end of sv info (this is an Nmea 4.1 Signal Id field)
					break;
				}

				if (GetItem(svInfoPos, pBuffer, iSize, svid))
				{
					if ((svid < 1) || (svid > 96))		// GPS + GLONASS + SBAS
					{
						// SV outside valid range, set to 0
						svid = 0;	
					}
					double d, az, el;
					...
					// cno
					if (GetItem(svInfoPos + 3, pBuffer, iSize, d) && (d > 0.0) && (d < 70.0))
					{
						pDatabase->Set(DATA_SATELLITES_IN_VIEW_STN_RATIO_(ixInView), d);
					}

					// el / az
					if (GetItem(svInfoPos + 1, pBuffer, iSize, el) && (el >= -90.0) && (el <= 90.0) &&
						GetItem(svInfoPos + 2, pBuffer, iSize, az) && (az >= -180.0) && (az <= 360.0))
					{
